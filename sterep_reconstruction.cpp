...
int main( int argc, char** argv )
{
 ...
 /* if (argc != 4)
  ...
  const char* QPath;
  QPath = "Q.xml";
  const char* rgbImageFile;
  //rgbImageFile = "rgb-image.ppm";
  rgbImageFile = "left13.ppm";
  const char* dispFile;
  //dispFile = "disparity-image.pgm";
  dispFile = "disparity.ppm";
  //Load Matrix Q
  cv::FileStorage fs(QPath, cv::FileStorage::READ);
  cv::Mat Q;
  ...
  std::cout << "Read matrix in file " << QPath << std::endl;
  ...
  */
  ...
  //Load rgb-image
  cv::Mat img_rgb = cv::imread(rgbImageFile, CV_LOAD_IMAGE_COLOR);
  if (img_rgb.data == NULL)
  {
    std::cerr << "ERROR: Could not read rgb-image: " << rgbImageFile << std::endl;
    system("PAUSE");
	return 1;
  }
  
  //Load disparity image
  cv::Mat img_disparity = cv::imread(dispFile, CV_LOAD_IMAGE_GRAYSCALE);
  if (img_disparity.data == NULL)
  {
    std::cerr << "ERROR: Could not read disparity-image: " << dispFile << std::endl;
    system("PAUSE");
	return 1;
  }
